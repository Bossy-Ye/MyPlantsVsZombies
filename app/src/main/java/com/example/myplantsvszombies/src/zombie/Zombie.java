...
public class Zombie extends CCSprite {
    ...
    private final GameLayer combatLayer;
    ...
    private String stand = Tools.zombieStand;
    private String move = Tools.zombieMove;
    private int moveInt = Tools.zombieMoveInt;
    private String attacka = Tools.zombieAttack;
    private int attackInt = Tools.zombieAttackInt;
    ...
    public Zombie (GameLayer combatLayer, CGPoint start, CGPoint end) {
        super(Tools.zombieStands);
    ...
    public Zombie (GameLayer combatLayer, CGPoint start, CGPoint end,int HP) {
        super(Tools.zombieStands);
        setAnchorPoint(0.5f, 0);
        setPosition(start);
        this.combatLayer = combatLayer;
        ...
        if (HP==600){
            ...
            setHP(400);
            setSpeed(15);
            move = Tools.bucketheadZombieMove;
            attacka = Tools.bucketheadZombieAttack;
            moveInt = Tools.bucketheadZombieInt;
            attackInt = Tools.bucketheadZombieAttackInt;
        }else if(HP==300) {
            isN = false;
            setHP(200);
            setSpeed(15);
            move = Tools.coneheadZombieMove;
            attacka = Tools.coneheadZombieAttack;
            moveInt = Tools.coneheadZombieInt;
            attackInt = Tools.coneheadZombieAttackInt;
        }else if(HP==190){
            isN = false;
            setHP(190);
            setSpeed(15);
            move = Tools.flagZombieMove;
            attacka = Tools.flagZombieAttack;
            moveInt = Tools.flagZombieInt;
            attackInt = Tools.flagZombieAttackInt;
        }
        move();
    }
...
//        runAction(ccBlink);
        ...
        if (!isN){
            if (HP<=100){
                move = Tools.zombieMove;
                attacka = Tools.zombieAttack;
                moveInt = Tools.zombieMoveInt;
                attackInt = Tools.zombieAttackInt;
                move();
            }
        }
...
//        runAction(ccBlink);
    ...
    public GameLayer getCombatLayer() {
        return combatLayer;
    }
